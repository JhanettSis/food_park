...
class PaymentController extends Controller
{
    ...
    {
        /**
         * All the colums in 'config('gatewaySettings.' is coming from
         * App\Services\PaymentGatewaySettingService.php
         * All is like the backend site
         * this code wi use the same from the documentation page
         * https://laravel-paypal.readthedocs.io/en/latest/installation.html#laravel-6-above
         * Configuration File
         * The configuration file paypal.php is located in the config folder.
         * Following are its contents when published:
         */
        $config = [
    ...
    {
        //return 'Payment Procesing';
        $config = $this->setPaypalConfig();
        /**
         * I use the configuration from the documentacion
         * https://laravel-paypal.readthedocs.io/en/latest/usage.html
         *Usage
         * Initialization
         * Usage
    ...
    function paypalSuccess(Request $request, OrderService $orderService)
    {
        ...
        $provider = new PayPalClient($config);
        /**
         * For this code I followed the the page documentation
         * https://laravel-paypal.readthedocs.io/en/latest/usage.html
         * Get Access Token
         * After setting the PayPal API configuration,
         * you need to get access token before performing any API calls
         * provider->getAccessToken();
         */
        $provider->getAccessToken();
        ...
        if(isset($response['status']) && $response['status'] === 'COMPLETED'){
            /**
             * This session is added in the file
             * App\Services\OrderServices.php
             * in the function createOrder
             */
    ...
    function payWithRazorpay(Request $request, OrderService $orderService) {
        ...
        if($request->has('razorpay_payment_id') && $request->filled('razorpay_payment_id')){
            ...
            if($response['status'] === 'captured'){
                ...
                $this->transactionFailUpdateStatus('Razorpay');
                return redirect()->route('payment.cancel')->withErrors('Something went wron with the payment!');
            }
        }
    }
}
